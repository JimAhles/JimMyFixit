...
function New-FirewallRuleForWebsite
{
    ...
    if (!($subscriptionID)) {throw "Error: Cannot get Azure subscription ID . Failed in New-FirewallRuleForWebsite in New-AzureSql.ps1"}
	if (!($thumbprint)) {throw "Error: Cannot get Azure thumbprint. Failed in New-FirewallRuleForWebsite in New-AzureSql.ps1"}
...
# jim a this script is consistently blowing up when using the context - so i'm changeing to NOT use it
...
$appdb = New-AzureSqlDatabase -ServerName $databaseServerName -DatabaseName $AppDatabaseName  -Verbose
...
$memberdb = New-AzureSqlDatabase -ServerName $databaseServerName -DatabaseName $MemberDatabaseName  -Verbose
